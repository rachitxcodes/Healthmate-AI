...
export default function RiskPredictor() {
  ...
  useEffect(() => {
    return () => {
      // Intentionally not revoking object URLs here for Base64 previews
    };
  }, []);
  ...
  const handleUploadAndExtract = async () => {
    ...
    try {
      ...
      await saveReportToHistory(newRecord);
      loadHistory(); // Refresh the list

      // Brief delay to show 100% before transition
      setTimeout(() => {
        setAppState("review");
  ...
  const handleConfirmAndAnalyze = () => {
    // Instead of doing the prediction here, we save the modified extraction
    // back into sessionStorage and push them to ReportResult to handle Step 2.
    ...
    <div className="w-full text-text-primary min-h-[calc(100vh-80px)]">
      <main className="mx-auto max-w-7xl px-4 sm:px-6 pt-8 pb-32 flex flex-col items-center">
        ...
        <div className={`w-full transition-all duration-700 ease-in-out flex ${appState === "review" ? "flex-col lg:flex-row gap-8 lg:items-start" : "justify-center"}`}>
          ...
          {/* SPLIT REVEAL: EXTRACTION FORM (RIGHT COLUMN) */}
          <AnimatePresence>
            {appState === "review" && (
              <motion.div
        ...
        {/* PREVIOUSLY UPLOADED REPORTS GALLERY */}
        <AnimatePresence>
            ...
            >
              <div className="mb-8 flex items-center justify-between">
                <div>
                  ...
                  <p className="text-slate-500 font-medium">Previously extracted reports securely stored on your device.</p>
                </div>
                ...
                {pastReports.map((report) => (
                  <Link to={`/report-history/${report.id}`} key={report.id} className="group outline-none">
                    <GlassCard className="!p-0 h-full overflow-hidden transition-all duration-300 group-hover:-translate-y-2 group-hover:shadow-[0_20px_40px_rgba(244,63,94,0.1)] group-focus-visible:ring-4 group-focus-visible:ring-rose-200">
                      <div className="h-40 w-full bg-slate-100 overflow-hidden relative border-b border-slate-100 group-hover:bg-rose-50 transition-colors">
                        {report.imageSrc ? (
                          ...
                          <div className="w-full h-full flex items-center justify-center text-slate-300 group-hover:text-rose-300 transition-colors">
                            <FileText size={48} />
                          </div>
                        )}
                      ...
                      <div className="p-6">
                        ...
                        <h3 className="font-bold text-slate-900 text-lg truncate mb-4 pr-4">{report.name || "Untitled Report"}</h3>
